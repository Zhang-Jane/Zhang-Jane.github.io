...
![MySQL Server 层与存储引擎分层示意](../images/mysql/mysql-server-layer-architecture.png)

![SQL 请求在 Server 层与引擎层的交互](../images/mysql/sql交互.png)
...
- MySQL 8.0 起已移除查询缓存：[MySQL 8.0: Retiring support for the query cache](https://mysqlserverteam.com/mysql-8-0-retiring-support-for-the-query-cache/)
...
![InnoDB 表空间与页、区、段关系示意](../images/mysql/表空间结构.webp)
...
![Compact 行格式示意](../images/mysql/compact格式.png)
...
- xx.ibd，表数据会保存在这个文件。表数据既可以存在共享表空间文件（文件名：ibdata1）里，也可以存放在独占表空间文件（文件名：表名字.ibd）。这个行为是由参数 innodb_file_per_table 控制的，若设置了参数 innodb_file_per_table 为 1，则会将存储的数据、索引等信息单独存储在一个独占表空间，从 MySQL 5.6.6 版本开始，它的默认值就是 1 了，因此从这个版本之后， MySQL 中每一张表的数据都存放在一个独立的 .ibd 文件。
  ![InnoDB Compact 相关结构示意](../images/mysql/InnoDB的compact.png)
...
![UPDATE 语句与 redo / binlog 两阶段提交示意](../images/mysql/update-two-phase-commit-flow.png)
...
![二叉查找树示意](../images/datastruct/二叉树.png)
...
![平衡二叉树高度与磁盘 I/O](../images/datastruct/二叉树的高度.png)
但是**不管平衡二叉查找树还是红黑树，都会随着插入的元素增多，而导致树的高度变高，这就意味着磁盘 I/O 操作次数多，会影响整体数据查询的效率**。

### N 叉树

![多叉树降低树高示意](../images/datastruct/三叉树.png)
...
![B+ 树结构示意](../images/datastruct/B+树.png)
...
![InnoDB B+ 树索引与聚簇/二级索引](../images/datastruct/Innodb的B+树.png)
...
![主键索引与二级索引的回表查询](../images/datastruct/索引查找.png)
...
InnoDB 删除部分行后，空间往往以**页内可复用空洞**等形式存在，**不会自动收缩** `.ibd`；需要 `OPTIMIZE TABLE`、`ALTER TABLE ... ENGINE=InnoDB` 或 `重建表` 等方式整理碎片（视版本与场景而定）。
...
- 为了在单个InnoDB表上执行多个并发写入操作时避免死锁，可以在事务开始时通过为预期要修改的每个元组（行）使用`SELECT ... FOR UPDATE`语句来获取必要的锁，即使这些行的更改语句是在之后才执行的。
...
延伸阅读：[分布式事务常见方案整理（知乎专栏）](https://zhuanlan.zhihu.com/p/183753774)
