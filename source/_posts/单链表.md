---
title: Python单链表的表示：单链表
description: Python单链表的表示：单链表。单链表 单链表是一种链式存取的数据结构，用一组地址任意的存储单元存放线性表中的数据元素。链表中的数据是以结点来表示的，每个结点的构成：元素(数据元素的映象) + 指针(指示后继元素存储位置…适合日常查阅、复习对照与工程检索。含命令、参数与常见踩坑提示。
date: 2020-10-14T19:44:04.000Z
tags: 数据结构
categories: 数据结构和算法
top_img: https://pic4.zhimg.com/ed93cb4a546971f4dfaaf31666dfb8b1_r.jpg?source=1940ef5c
cover: https://w.wallhaven.cc/full/39/wallhaven-39xqzy.jpg
abbrlink: 95221c2a
