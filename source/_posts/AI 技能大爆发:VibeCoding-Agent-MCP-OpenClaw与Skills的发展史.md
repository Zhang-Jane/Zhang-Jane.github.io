...
Anthropic 在 2024 年 11 月前后开源并推广了 [Model Context Protocol（MCP）](https://modelcontextprotocol.io/)：目标很直接——少做「每个产品各造一套 AI 接数据库、接 SaaS、接本地文件」的碎片活。可以把它想成：**在应用与数据源、工具之间，约定一种双向、可发现、可安全管控的连接方式**（常被比作「AI 侧的通用接口」）。

MCP Server 一旦把 Git、日历、知识库等封装好，Agent 就不必为每个集成从零写 schema；同一套能力也能在多款客户端里复用。2025 年起，主流厂商陆续支持 MCP 或同类互操作方案，生态从「Anthropic 一家推」走向多方参与。
...
> 有一种我称之为“Vibe Coding”的新编程方式，即完全沉浸在氛围中，拥抱指数级增长，甚至忘记代码的存在。之所以能做到这一点，是因为大型语言模型（例如 Cursor Composer 搭配 Sonnet）已经变得太强大了。此外，我只需通过 SuperWhisper 与 Composer 对话，几乎不用碰键盘。我会提出“把侧边栏的内边距减半”这种最蠢的要求，因为我懒得去查找。我总是“全盘接受”，不再阅读代码差异。遇到错误信息时，我直接复制粘贴进去，不加任何说明，通常这样就能解决问题。代码规模已经超出了我通常的理解范围，我得花些时间仔细阅读才能搞懂。有时大语言模型（LLM）无法修复某个 bug，我就直接绕过它，或者让它随意改改直到问题消失。对于那些周末随手搞的临时项目来说，这倒也没什么大不了的，但确实挺有意思。我虽然在构建一个项目或 Web 应用，但这其实算不上真正的编程——我只是看着代码、下达指令、运行程序，再复制粘贴，结果大多时候都能正常工作。

约 2025 年 2 月，Andrej Karpathy 在 [X](https://x.com/karpathy/status/1886192184808149383) 上把一种做法叫 **Vibe Coding**：更靠「感觉」和意图描述，用自然语言（甚至语音）让模型生成、修改、调试代码，实践上往往**少抠每一行手写、多靠迭代指令**。这能成立，前提是编辑器 + 强模型（例如带 Composer 类工作流的工具）已经能扛住大量实现细节。
...
2025 年下半年起，社区里由 Peter Steinberger 等推动的开源项目关注度很高：早期叫过 Clawdbot，后因品牌与商标等原因改名，路线逐渐收束到 **OpenClaw**——强调**可本地或自托管**，并可通过 Telegram、WhatsApp、Discord 等驱动，支持命令行、文件、记忆与多 Agent 协作。仓库曾在短时间内冲到很高的 star（具体数以当时页面为准）。
...
| **主要功能**                                         | - 作为“主机”运行 Agent - 连接聊天 App 接收指令 - 管理内存、会话、心跳（主动唤醒）                                                    |
| - 支持子 Agent / 多 Agent 协作 - 集成 MCP 或内置工具 | - 封装特定工作流（如 GitHub PR 审查、邮件处理、日历同步） - 提供自然语言指令、触发条件、最佳实践 - 支持动态加载、沙箱执行、自进化    |
...
| **优势**                                             | 本地隐私安全、无持续 API 费用、支持聊天直接指挥、病毒式传播（GitHub 星标极高）                                                       | 即插即用、轻量高效、社区生态庞大（ClawHub 超 13,000 个技能）、易于编写（非代码，类似写文档）               |
...
在 OpenClaw 这条线上，**Skills** 常被说成杀手级特性：每个 Skill 是一个约定好的目录（例如含 `SKILL.md`、元数据与指令），可被动态加载、在受控环境里执行，并分享到社区注册表。[ClawHub](https://clawhub.ai/)站点负责「发现与版本」——这和我在 [Agent Skills：AI 时代的可复用能力封装](/post/cd4fa45f.html) 里写的「把经验打包给智能体读」是同一思想在不同产品里的体现：一边是通用 Agent Skills 规范与 IDE 生态，一边是 OpenClaw 系里的 Skill 市场。
...
| 时段         | 事件要点                                          |
| ------------ | ------------------------------------------------- |
| 2024.11 前后 | Anthropic 推动 MCP，工具连接开始协议化            |
| 2025.02 前后 | Karpathy 提出 Vibe Coding，自然语言驱动开发成热词 |
