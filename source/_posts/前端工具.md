---
title: 前端工具：nvm、node、npm 、yarn区别
description: 前端工具：nvm、node、npm 、yarn区别。nvm、node、npm 、yarn区别 1. nvm：nodejs 版本管理工具，也就是说：一个 nvm 可以管理很多 node 版本和 npm 版本。 2. nodejs：在项目开发时…适合日常查阅、复习对照与工程检索。含命令、参数与常见踩坑提示。
