...
description: 如何调研一款新的互联网 SaaS 产品是否值得去做：一、市场需求（需求度）——确认痛点真实且强烈 理论解释：SaaS 成败第一关是「问题是否足够痛」。用户画像 + Y 模型上半部验证「谁、何时、何地、做什么」（场景）→「为什么」（目标/动机…适合日常查阅、复习对照与工程检索。含命令、参数与常见踩坑提示。
