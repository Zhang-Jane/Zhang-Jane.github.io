...
用于 `source/_posts/*.md` 文末 `## 引用来源` 与正文上标注脚的发布前自检。

- **连通性必检**：引用链接必须可访问（避免 404/410/5xx 等失效链接）。
- **重定向必检**：若最终落地页与引用描述不一致（如跳转到首页、聚合页或无关页面），视为失真引用，不可保留。
- **语义一致性**：引用描述应与目标页面核心内容一致；不得用“博客首页/分类页”承载具体事实性陈述，除非该条本身就是“合集/索引说明”。
- **原始来源优先**：优先使用官方文档、原始公告、庭审记录、论文、作者原文或一手发布页；二手转述仅作补充。
- **无等价来源即删除**：当原链接失效且无法找到“可访问 + 语义等价”的原始来源时，该条引用不展示；同时移除正文对应注脚锚点，避免悬空引用。
- **最小改动原则**：修复引用时优先替换 URL 并保持注号稳定（`〔注n〕` / `fn-notes{n}`）；仅在必要时调整描述文本以匹配新来源。

## 五、数学公式书写规范（MathJax）

- 涉及数学公式时，优先使用 **块级公式**：`$$ ... $$`（与站内既有文章保持一致）
- 禁止混用 `$...$` 行内公式与 `\(...\)` 行内公式
- 文中变量名（如 `T*`、`Q*`、`P*`）若仅作术语出现，使用普通文本（必要时对 `*` 转义为 `\*`），不强制写成行内公式
- 发布前本地预览确认公式渲染正常（Hexo + Butterfly + MathJax）
